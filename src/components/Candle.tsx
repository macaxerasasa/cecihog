...
/* Shared paint servers. Kept in the document (not display:none) so every
   flame can point at the same gradients instead of cloning SVG filters. */
export function FlameDefs() {
  return (
    <svg className="flame-defs" aria-hidden="true">
      <defs>
        ...
        <radialGradient id="fl-blue" cx="0.5" cy="0.7" r="0.5">
  ...
  return (
    <svg className={`flame-svg ${className}`} viewBox="0 0 60 60" aria-hidden="true">
      <g className="candle-halo">
        <circle cx="30" cy="40" r="28" fill="url(#fl-halo)" />
      </g>
      <g className="candle-flame">
        <ellipse cx="30" cy="40" rx="9" ry="15" fill="#ff8c2a" opacity="0.35" />
        <path
        ...
        <path
          ...
          opacity="0.95"
        />
        <ellipse cx="30" cy="50" rx="5" ry="4.5" fill="url(#fl-blue)" />
      </g>
    </svg>
  )
})
