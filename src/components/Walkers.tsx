  ...
  return (
    <g className="walkers" aria-hidden="true">
      {list.map((w, i) => {
        ...
        return (
          <g key={w.name} className="walker" style={style}>
            {!lite
              ? TRAIL.map((o, k) => (
                  <g
                    key={k}
                    className="walker-feet is-trail"
                    style={{ '--lag': `${-(k + 1) * 1.05}s`, opacity: o } as CSSProperties}
                  >
                    <Feet />
                  </g>
                ))
              : null}
            <g className="walker-feet">
              <Feet />
            </g>
            <g className="walker-tag">
              <path
                className="tag-ribbon"
                d={ribbon(w.name.length * 9.2 + 26, 24)}
                transform={`translate(${-(w.name.length * 9.2 + 26) / 2}, -52)`}
              />
              <text className="tag-name" x="0" y="-35" textAnchor="middle">
                {w.name}
              </text>
            </g>
          </g>
